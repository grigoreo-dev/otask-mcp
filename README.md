...
[MCP-сервер](https://modelcontextprotocol.io) для [O!task API](https://api.otask.ru/docs). Отдаёт операции O!task (пространства/задачи) как MCP-инструменты для агентов (Claude, Cursor, OpenCode, n8n MCP Client Tool).
...
Multi-user OAuth, `OTASK_*` в env Worker нет. Self-host: см. [🔧 Self-deploy Worker](#-self-deploy-worker).
...
Явные `ws_slug` / project args в tools по-прежнему работают. Self-host stdio/HTTP: env `OTASK_DEFAULT_*`. Про UUID-slug vs `#N` на доске: [`docs/SLUGS.md`](docs/SLUGS.md).
...
| `OTASK_DEFAULT_WS` | stdio, HTTP | опционально | Slug пространства по умолчанию (UUID; если tool не передал `ws_slug`) |
| `OTASK_DEFAULT_PROJECT` | stdio, HTTP | опционально | Project slug (UUID) **или** numeric id по умолчанию |
| `OTASK_ALLOWED_WS` | stdio, HTTP gateway | опционально | Allow-list slug пространств через запятую |
...
| `X-Otask-Allowed-Ws` | **только passthrough** | Allow-list пространств |
| `X-Otask-Default-Ws` | **passthrough** (override env) | Default slug пространства |
...
| `otask_list_workspaces` | Список **пространств** (teams); без slug; при одном пространстве агент/резолвер может подставить его автоматически |
| `otask_list_tasks` | Задачи пространства; по умолчанию `mine=true`; фильтры `performer_ids`, `project_ids`, `priority_ids`, `due`, `page` |
| `otask_get_task` | Получить одну задачу по пространству + slug задачи |
| `otask_update_task` | Обновить существующую задачу |
| `otask_list_projects` | Список проектов пространства (с фильтром по allow-list) |
...
| `otask_list_members` | Участники пространства |
| `otask_list_tags` | Теги пространства |
...
Про UUID-slug vs номера досок `#N`: [`docs/SLUGS.md`](docs/SLUGS.md).
...
|-----|---------------------|----------------------|
| Default пространство | `OTASK_DEFAULT_WS` | `X-Otask-Default-Ws` (override) |
| Default project | `OTASK_DEFAULT_PROJECT` (slug или id) | `X-Otask-Default-Project` |
| Limit пространств | `OTASK_ALLOWED_WS` | `X-Otask-Allowed-Ws` |
| Limit projects | `OTASK_ALLOWED_PROJECTS` | `X-Otask-Allowed-Projects` |

Формат allow-list: значения через запятую. Projects: **slug** (UUID) и/или **numeric id**. Пространства: только slug (UUID). **Пусто = без ограничения** (off). Подробнее: [`docs/SLUGS.md`](docs/SLUGS.md).
